...
class Message(object):
    ...
    def apply_shift(self, shift):
        ...
        '''
        # create empty list with the new characters
        shifted = []

        # so we go over the message_text, fill each character with the value from the new dictionary
        for c in self.message_text:
            if c not in self.build_shift_dict(shift).keys():
                shifted.append(c)
            else:
                shifted.append(self.build_shift_dict(shift)[c])
        return ''.join(shifted)
        ...
        for i in range(26):
            ...
            for w in words:
                if is_word(self.valid_words, w):
                    found_words += 1
            if found_words > max_words:
                max_words = found_words
                best_i = i
                decrypted_text = temp

        return (best_i, decrypted_text)
